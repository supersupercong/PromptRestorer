...
class FeMaSRModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.LQ_stage:
            with torch.no_grad():
                self.gt_rec, feature_degradation, restoration = self.net_hq(self.lq)
    ...
    def test(self):
        ...
        lq_input = self.lq
        gt_rec, feature_degradation, restoration = self.net_hq(self.lq)
        _, _, h, w = lq_input.shape
        if h * w < min_size:
            # out_img, feature_degradation, self.output = self.net_g(self.lq, feature=feature_degradation)
            self.output = net_g.test(lq_input, feature=feature_degradation)
        else:
            self.output = net_g.test_tile(lq_input, feature=feature_degradation)
        self.net_g.train()
